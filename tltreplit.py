...
def fight(enemyhealth, weapon, monster):
  ...
  if hitcrit == False:
    #did not hit crit, different message
    ...
    #did hit crit, different message
  ...
  #decrement stamina
  if stamina <= 0:
    #collapse if stamina is depleted during a fight
...
def move(direction, position):
  ...
  try:
    if direction == "back":
      ...
      if position.left.level > furthestlevel:
        furthestlevel = position.left.level
      ...
      if position.right.level > furthestlevel:
        furthestlevel = position.left.level
...
def campfire():
  decision = takeinput("What would you like to do at the campfire?", "rest", "shop", "move", "peek", "stats")
  if decision.lower() == "shop":
    shop()
  elif decision.lower() == "rest":
    rest()
  elif decision == "stats":
    print(f"Current level: {position.level}")
    print(f"Furthest level: {furthestlevel}")
    print(f"Total enemies defeated: {enemiesdefeated}")
  elif decision == "move":
    return "move"
  else:
    return "peek"
  ...
  for i in range(5):
    ...
    if timepassed > 2:
      ...
      if enemyhealth == 0:
        ...
        print("you won!")
        time.sleep(0.5)
        yesorno = takeinput("See your stats?", "yes", "no")
        if yesorno == "yes":
          print(f"Current level: {position.level}")
          print(f"Furthest level: {furthestlevel}")
          print(f"Total enemies defeated: {enemiesdefeated}")
...
time.sleep(3)
helpmessage_y_n = input("Go through tutorial? y/n ")
# y = yes, n = no
if helpmessage_y_n == "y":
  print("""
  How to play the game:
  Your aim is to get to the end of the dungeon and defeat the final boss.
  Each room can contain one of three things: a campfire, a monster, or a chest.
  """)
  input("Press enter to continue.")
  print("""
  The campfire is the "checkpoint" of the game - 
      rest can be used to heal,
      shop can be used to buy better equipment with your gold,
      move can be used to move to the left, move to the right, or move back,
      peek can be used to peek at the contents of the room on your left or the room on your right,
      and stats can be used to see your current level, the furthest level you've reached, and the amount of enemies you've defeated.
  """)
  input("Press enter to continue.")
  print("""
  A monster is something that you have to defeat - 
      upon engaging a monster, you MUST defeat it before moving on,
      fight can be used to fight the monster with your weapon,
      and heal can be used to heal yourself for 30% of your max hp.
  Monsters can also drop 3 - 5 gold when killed.
  """)
  input("Press enter to continue.")
  print("""
  A chest is something that you can choose to open -
      opening a chest will give you 5 - 10 gold,
      unopened chests can be opened when you return to them.
  """)
  input("Press enter to continue.")
  print("""
  Stamina is used when you move (2), peek (1), or attack an enemy (1). If you run out of stamina mid fight you will collapse and lose.
  If you run out of stamina while peeking or moving, you will collapse and wake up with half stamina and half of your old health.

  The final boss room is at the 9th level. The starting level, or 0th level, is a campfire. Can you make it to the end? Good luck.
  """)
